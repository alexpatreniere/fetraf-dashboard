// src/app/dashboard/usuarios/Client.tsx
"use client";
...
function compareBy(field: keyof Usuario, dir: Dir) {
...
export default function UsuariosClient() {
  ...
  const sort = (searchParams.get("sort") as keyof Usuario) || "nome";
  ...
  function pushQuery(next: Partial<Record<string, string | number | undefined>>) {
    ...
    router.push(`/dashboard/usuarios${usp.toString() ? `?${usp}` : ""}`);
  }
  ...
  function onHeaderClick(field: keyof Usuario) {
  ...
  const th = (label: string, field: keyof Usuario) => (
    <th
      ...
      <PageHeader
        ...
        actions={
          <div className="flex w-full items-center gap-2 sm:w-auto">
            ...
            <a href="/dashboard/usuarios/novo" className="btn btn-brand">
              Novo Usuário
            </a>
          </div>
        }
      ...
      <div className="card p-0 overflow-x-auto">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((u) => (
                <tr key={u.id} className="hover:bg-[var(--surface-3)] border-b border-[var(--border)]">
                  <td className="py-2.5 px-2">
                    <a className="hover:underline" href={`/dashboard/usuarios/${u.id}`}>{u.nome}</a>
                  </td>
                  <td className="py-2.5 px-2">{u.email ?? "-"}</td>
