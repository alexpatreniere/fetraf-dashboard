// src/app/dashboard/relatorios/Client.tsx
"use client";
...
type Relatorio = {
  id: string | number;
  titulo: string;
  tipo?: string;            // ex.: "PDF", "Planilha"...
  criadoEm?: string;        // ISO date
  status?: string;          // ex.: "Pronto", "Processando", "Erro"
  tamanho?: number | string; // bytes ou string legível
};
...
function compareBy(field: keyof Relatorio, dir: Dir) {
  return (a: Relatorio, b: Relatorio) => {
    if (field === "criadoEm") {
      const da = a.criadoEm ? Date.parse(a.criadoEm) : 0;
      const db = b.criadoEm ? Date.parse(b.criadoEm) : 0;
      return cmp(da, db, dir);
    }
    if (field === "tamanho") {
      return cmp(toNumber(a.tamanho), toNumber(b.tamanho), dir);
    }
    return (
      safeStr(a[field]).localeCompare(safeStr(b[field]), "pt-BR", {
        sensitivity: "base",
      }) * (dir === "asc" ? 1 : -1)
    );
  };
...
export default function RelatoriosClient() {
  ...
  const sort = (searchParams.get("sort") as keyof Relatorio) || "criadoEm";
  ...
  // metadados (quando o backend enviar)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const metaPage =
          body?.page ?? body?.meta?.page ?? body?.pagination?.page ?? page;
        ...
        const metaHasMore =
          ...
          body?.meta?.hasMore ??
          (metaTotal != null
            ? metaPage * metaLimit < metaTotal
            : list.length === metaLimit);

        // se não veio paginação/ordenação do backend, faz no cliente
        let finalList = list;
        if (metaTotal == null) {
  ...
  function onHeaderClick(field: keyof Relatorio) {
  ...
  const th = (label: string, field: keyof Relatorio) => (
    <th
    ...
    <PageLayout>
      <PageHeader
        ...
        actions={
          <div className="flex w-full items-center gap-2 sm:w-auto">
            ...
            <a href="/dashboard/relatorios/novo" className="btn btn-brand">
              Novo Relatório
            </a>
          </div>
        }
      ...
      <div className="card p-0 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-6 text-sm muted">Nenhum relatório encontrado.</div>
        ) : (
            ...
            <tbody>
              {data.map((r) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-2">{r.tipo ?? "-"}</td>
                  <td className="py-2.5 px-2">
                    <span className="chip">{r.status ?? "-"}</span>
                  </td>
                  <td className="py-2.5 px-2">
                    {r.tamanho != null ? String(r.tamanho) : "-"}
                  </td>
                  <td className="py-2.5 px-2 muted">
                    {r.criadoEm ? new Date(r.criadoEm).toLocaleString() : "-"}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={
              hasMore === false ||
              (total != null && pageCount != null && page >= pageCount)
            }
          >
            Próxima
          </button>
