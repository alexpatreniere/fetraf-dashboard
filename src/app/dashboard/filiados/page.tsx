...
// (opcional) se você criou estes componentes conforme combinamos:
...
export default function FiliadosPage() {
  ...
  const th = (label: string, field: keyof Filiado) => (
    <th
      className="py-2.5 px-2 text-left font-medium cursor-pointer select-none border-b border-[var(--border)] muted"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((f) => (
                <tr
                  key={f.id}
                  className="hover:bg-[color-mix(in_oklch,var(--surface),black_4%)] border-b border-[var(--border)]"
                >
